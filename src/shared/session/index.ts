...
export const sessionRequestFx = attach({effect: api.sessionFx});
...
export function chainAuthorized<Params extends RouteParams>(
  ...
  sample({
    clock: sessionCheckStarted,
    source: $authenticationStatus,
    filter: (status: any) => status === AuthStatus.Initial,
...
export function chainAnonym<Params extends RouteParams>(
  route: RouteInstance<Params>,
  {otherwise}: ChainParams<Params> = {},
): RouteInstance<Params> {
  const sessionCheckStarted = createEvent<RouteParamsAndQuery<Params>>();
